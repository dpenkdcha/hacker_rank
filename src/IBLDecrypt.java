...
public class IBLDecrypt {
	...
	public static void main(String[] args) throws Exception {
		...
		//System.out.println("Sending --> "+encryptedText);
		String decrypted="";
		byte[] decoded = Base64.getDecoder().decode(URLDecoder.decode(encodedEncryptedText));
		//byte[] decoded = Base64.getDecoder().decode(encryptedText);
		decrypted = decrypt(decoded);
		System.out.println("Receving --> "+decrypted);
		...
		/*
		 * String secret = "tata123456789123"; String cipherText =
		 ...
		 * Arrays.copyOfRange(cipherData, 8, 16); MessageDigest md5 =
		 * MessageDigest.getInstance("MD5"); final byte[][] keyAndIV =
		 ...
		 * IvParameterSpec(keyAndIV[1]); byte[] encrypted =
		 * Arrays.copyOfRange(cipherData, 16, cipherData.length); Cipher aesCBC =
		 * Cipher.getInstance("AES/CBC/PKCS5Padding"); aesCBC.init(Cipher.DECRYPT_MODE,
		 * key, iv); byte[] decryptedData = aesCBC.doFinal(encrypted); String
	...
	private static String decrypt(byte[] encryptedText) throws Exception {
	    ...
	    byte[] keyBytes = "9DC3D4B582CC0445".getBytes();
